...
router.get('/:page/:size/:sort_field/:sort_dir/:role', function (req, res, next) {
  ...
  console.log(page, size, sort_field, sort_dir, role)
  var log = { page: '', total: '', size1: '', data1: '' };
  if (!sort_field == ' ') {
    User.paginate(User.find({ role: role }).sort({ [sort_field]: sort_dir, test: test }), { page: page, limit: size }, function (err, users) {
    ...
    User.paginate(User.find({ role: role }), { page: page, limit: size }, function (err, users) {
      if (err) return next(err);
      log.page = users.page;
      log.total = users.total;
      log.size1 = users.docs.length;
